...
const App = () => {
  ...
  const handleProcessQueue = () => {
    ...
    const code = selectedPoli === 'Umum' ? 'A' : selectedPoli === 'Gigi' ? 'B' : 'C';
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xl font-bold tracking-tight">Klinik As-Syifa</span>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
          ...
          <div className="p-6 md:p-8">
            <div className="flex flex-col md:flex-row md:items-end gap-6">
              <div className="flex-1">
                <label className="block text-sm font-semibold text-slate-600 mb-2">Cek Pasien Lama (NIK / Nama)</label>
        ...
        {showAddForm && (
          <div className="bg-white rounded-2xl shadow-md border-2 border-emerald-100 mb-8 animate-in slide-in-from-top-4 duration-300 overflow-hidden">
            <div className="bg-emerald-50 px-6 py-4 border-b border-emerald-100 flex justify-between items-center">
              ...
              <span className="text-xs text-emerald-600 bg-emerald-100 px-2 py-1 rounded-full font-semibold uppercase tracking-wider">Lengkapi Data</span>
            </div>
            <div className="p-6 md:p-8">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="text" className="w-full p-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Tanggal Lahir</label>
                      <input type="date" className="w-full p-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none" />
                    </div>
                  </div>
                </div>
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="flex gap-4 mt-2">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="gender" className="w-4 h-4 text-emerald-600 focus:ring-emerald-500" />
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Nomor Handphone/WhatsApp *</label>
                    <input type="text" className="w-full p-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none" placeholder="08xxxxxxxx" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Alamat Lengkap</label>
                    <textarea className="w-full p-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none h-[108px]" placeholder="Nama jalan, RT/RW, Kecamatan..."></textarea>
                  </div>
                </div>
              </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-y-scroll h-96">
          <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            ...
            <span className="text-xs font-medium text-slate-500">Menampilkan {filteredPatients.length} Data</span>
          </div>
                ...
                {filteredPatients.length > 0 ? (
                  filteredPatients.map((p) => (
                    <tr key={p.id} className="hover:bg-emerald-50/40 transition">
                      ...
                      <td className="px-6 py-4 font-mono text-xs text-slate-600">{p.nik}</td>
          ...
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            {!queueNumber ? (
              <>
                ...
                <div className="p-6 space-y-4">
                  <div className="space-y-2">
                    <label className="block text-xs font-bold text-slate-400 uppercase">Pilih Poli Klinik</label>
                    <div className="grid grid-cols-1 gap-2">
                      {['Umum', 'Gigi', 'KIA'].map((poli) => (
                        ...
                        >
                          ...
                          {selectedPoli === poli && <CheckCircle2 className="text-emerald-500" size={20} />}
                        </button>
                      ))}
                    </div>
                  </div>
                  <div className="flex gap-3 pt-4">
                    <button onClick={closeModals} className="flex-1 py-3 font-bold text-slate-500 hover:bg-slate-50 rounded-xl">Batal</button>
                    <button 
              ...
              <div className="p-8 text-center bg-white relative">
                ...
                <p className="text-slate-500 text-sm mb-6">Nomor antrean telah berhasil dicetak</p>
                
                <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl p-6 mb-6">
                  <div className="text-xs font-bold text-slate-400 uppercase mb-2 tracking-widest">Nomor Antrean</div>
                  <div className="text-6xl font-black text-emerald-700 mb-2">{queueNumber}</div>
                  <div className="text-sm font-bold text-slate-700 uppercase">POLI {selectedPoli.toUpperCase()}</div>
